...
// async function checkExistingTableList(phone) {
//   const connection = await pool.getConnection();

//   try {
//     const [rows] = await connection.query('SELECT * FROM tableList WHERE phone = ?', [phone]);
//     return rows.length > 0;
//   } finally {
//     connection.release();
//   }
...
// async function hasVacancy(restaurantId, headcount) {
//   const connection = await pool.getConnection();
...
// async function updateTableVacancy(phone) {
//   const connection = await pool.getConnection();

//   try {
//     await connection.query('UPDATE tableList SET vacancy = false WHERE phone = ?', [phone]);
//   } finally {
//     connection.release();
//   }
// }
...
// async function getWaitCount(restaurantId) {
//   const connection = await pool.getConnection();

//   try {
//     const [rows] = await connection.query('SELECT COUNT(*) AS waitCount FROM Reservation WHERE restaurantId = ?', [
//       restaurantId
//     ]);
//     return rows[0].waitCount;
//   } finally {
//     connection.release();
//   }
// }
