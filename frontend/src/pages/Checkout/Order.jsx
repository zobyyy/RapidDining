...
export default function Order( {order} ) {
    ...
    return (
        <div>
            {order?.status === "內用"
                ?
                   (order && <DineIn order={order} selectedOrderId={selectedOrderId} setSelectedOrderId = {setSelectedOrderId} />)
                :
                    (order && <DineOut order={order} selectedOrderId={selectedOrderId} setSelectedOrderId = {setSelectedOrderId} />)
            }
        </div>
    )
}
...
function DineIn ( { order, selectedOrderId, setSelectedOrderId } ) {
    return (
        <div className={styles.order} onClick={()=>setSelectedOrderId(order.orderId)}>
            <div className={styles.block} style={{width:'65%'}}>
                <p>桌號</p>
